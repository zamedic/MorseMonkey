...
public class AddRecipeCookbook implements Commandlet {
  ...
  public boolean canHandleCommand(Message message, State state) {
    return state.equals(State.CONFIGURE_EDIT_APPLICATION_RECIPE_COOKBOOK);
  }
  ...
  public void handleCommand(Message message, State state, List<String> parameters,
    ...
    applicationService.addRecipeToApplication(application,description,cookbook);
    morseBot.sendMessage("Added cookbook "+cookbook+" with description "+description+" to application "+application,message.getChatId().toString());
  }
